...
class EconomicGameApp:
    def __init__(self, root):
       ...
       # screen_width = self.root.winfo_screenwidth()
        ...
        self.graph_frame.grid(row=3, column=0, rowspan= 3, columnspan=3, pady=10, sticky="nsew")

        self.create_graph_panel()

        # News Feed
        self.news_frame = ttk.LabelFrame(self.main_frame, text="News Feed", padding="10", style="Green.TLabelframe")
        self.news_frame.grid(row=7, column=0, pady=10, sticky=(tk.W, tk.E))

        self.news_text = tk.Text(self.news_frame, font=("Helvetica", 12), wrap="word", background="white")
        self.news_text.grid(row=0, column=0, rowspan= 3, columnspan=3, sticky=(tk.W, tk.E))
        ...
        self.rate_label.grid(row=8, column=0, sticky=tk.E)

        self.rate_entry = ttk.Entry(self.main_frame, width=20, font=("Helvetica", 30), style="TEntry")
        self.rate_entry.grid(row=8, column=1, sticky=tk.W)

        # Next Button (Bottom Right)
        self.next_button = ttk.Button(self.main_frame, text="Next",
                                      command=self.next_turn)
        self.next_button.grid(row=9, column=2, sticky=tk.W)
