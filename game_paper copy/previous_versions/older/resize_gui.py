...
class EconomicGameApp:
    def __init__(self, root):
        ...
        # Title Label
        self.title_label = ttk.Label(self.main_frame, text="Economic Simulation Game", style="Green.TLabel")
        self.title_label.grid(row=0, column=0, columnspan=3, pady=5)

        # Quarter Label
        self.quarter_label = ttk.Label(self.main_frame, text=f"Quarter: {self.economy.current_quarter}", style="Green.TLabel")
        ...
        self.graph_frame.grid(row=3, column=0, columnspan=3, pady=10, sticky=(tk.W, tk.E))

        self.create_graph_panel()

        # News Feed
        self.news_frame = ttk.LabelFrame(self.main_frame, text="News Feed", padding="10", style="Green.TLabelframe")
        self.news_frame.grid(row=4, column=0, columnspan=3, pady=10, sticky=(tk.W, tk.E))

        self.news_text = tk.Text(self.news_frame, height=11, width=220, font=("Helvetica", 12), wrap="word", background="white")
        self.news_text.grid(row=0, column=0, columnspan=3, sticky=(tk.W, tk.E))

        # Input Field for New Interest Rate (Top Right)
        self.rate_label = ttk.Label(self.main_frame, text="Enter New Interest Rate:", padding="10", style="Info.TLabel")
        self.rate_label.grid(row=5, column=0, sticky=tk.E)

        self.rate_entry = ttk.Entry(self.main_frame, width=20, style="TEntry")
        self.rate_entry.grid(row=5, column=1, sticky=tk.W)

        # Next Button (Bottom Right)
        self.next_button = ttk.Button(self.main_frame, text="Next",
                                      command=self.next_turn)
        self.next_button.grid(row=6, column=2, sticky=tk.W)
        ...
        # Bind the resize event to update the figure size and fonts
    ...
    def create_graph_panel(self):
        # Initial figure size with a fixed aspect ratio
        aspect_ratio = 4 / 3  # Width to height ratio
        initial_width = 6  # Initial width in inches
        initial_height = initial_width / aspect_ratio

        self.fig, self.ax = plt.subplots(figsize=(initial_width, initial_height))
    ...
    def show_event_details(self, event):
        ...
        event_text = tk.Text(event_window, height=10, width=60, font=("Helvetica", 25), wrap="word", background="white")
    ...
    def on_window_resize(self, event):
        # Adjust the figure size based on the window size while maintaining the aspect ratio
        aspect_ratio = 4 / 3  # Width to height ratio
        window_width = self.root.winfo_width()
        window_height = self.root.winfo_height()

        # Calculate new figure size based on window size
        fig_width = window_width / 100  # Convert pixels to inches (approximate)
        ...
        base_font_size = int(window_width / 40)  # Adjust the divisor to scale font size
        self.title_label.config(font=("Helvetica", base_font_size + 10))
        self.quarter_label.config(font=("Helvetica", base_font_size))
        self.inflation_label.config(font=("Helvetica", base_font_size))
        self.unemployment_label.config(font=("Helvetica", base_font_size))
        self.interest_rate_label.config(font=("Helvetica", base_font_size))
        self.rate_label.config(font=("Helvetica", base_font_size))
        self.rate_entry.config(font=("Helvetica", base_font_size))
